...
def create_bmp5xx_class(robit_class, rwbit_class, robits_class, rwbits_class):
    class BMP5XX:
        chip_id: int = robits_class(8, BMP5_REG_ID, 0)

        # Status register bits
        status_nvm_ready: bool = robit_class(BMP5_REG_STATUS, 1)  # NVM ready
        """True if NVM is ready."""

        status_nvm_err: bool = robit_class(BMP5_REG_STATUS, 2)  # NVM error
        """True if NVM is has an error."""

        int_status_por: bool = robit_class(BMP5_REG_INT_STATUS, 4)  # INT_STATUS por
        """True if power on, or software reset complete."""

        data_ready_int_en: bool = rwbit_class(BMP5XX_REG_INT_SOURCE, 0)
        """Set to True to enable data_ready interrupt register."""

        fifo_full_int_en: bool = rwbit_class(BMP5XX_REG_INT_SOURCE, 1)
        """Set to True to enable FIFO full interrupt register."""

        fifo_threshold_int_en: bool = rwbit_class(BMP5XX_REG_INT_SOURCE, 2)
        """Set to True to enable FIFO threshold interrupt register."""

        pressure_oor_int_en: bool = rwbit_class(BMP5XX_REG_INT_SOURCE, 3)
        """Set to True to enable pressure OOR interrupt register."""

        data_ready: bool = robit_class(BMP5_REG_INT_STATUS, 0)
        """True if data is ready."""

        fifo_full_interrupt: bool = rwbit_class(BMP5_REG_INT_STATUS, 1)
        """True when FIFO is full."""

        fifo_threshold_interrupt: bool = rwbit_class(BMP5_REG_INT_STATUS, 2)
        """True when FIFO reached threshold."""

        pressure_oor_interrupt: bool = rwbit_class(BMP5_REG_INT_STATUS, 3)
        """True when pressure is OOR."""

        _temperature = robits_class(24, BMP5XX_REG_TEMP_DATA_XLSB, 0, 3)
        _pressure = robits_class(24, BMP5XX_REG_PRESS_DATA_XLSB, 0, 3)
        _mode = rwbits_class(2, BMP5XX_REG_ODR_CONFIG, 0)

        deep_disabled = rwbit_class(BMP5XX_REG_ODR_CONFIG, 7)
        """Deep standby disabled"""

        pressure_enabled = rwbit_class(BMP5XX_REG_OSR_CONFIG, 6)
        """Pressure readings enabled"""

        pressure_oversampling_rate = rwbits_class(3, BMP5XX_REG_OSR_CONFIG, 3)
        """Pressure oversampling rate. Must be one of the OVERSAMPLING constants."""

        temperature_oversampling_rate = rwbits_class(3, BMP5XX_REG_OSR_CONFIG, 0)
        """Temperature oversampling rate. Must be one of the OVERSAMPLING constants."""

        pressure_iir_filter = rwbits_class(3, BMP5XX_REG_DSP_IIR, 3)
        """Pressure IIR Filter. Must be one of the IIR_FILTER constants."""

        temperature_iir_filter = rwbits_class(3, BMP5XX_REG_DSP_IIR, 0)
        """Temperature IIR Filter. Must be one of the IIR_FILTER constants."""

        pressure_shadow_iir = rwbit_class(BMP5XX_REG_DSP_CONFIG, 5)
        """Pressure shadow IIR order. Must be BMP5XX_BEFORE_IIR_FILTER or BMP5XX_AFTER_IIR_FILTER"""

        temperature_shadow_iir = rwbit_class(BMP5XX_REG_DSP_CONFIG, 3)
        """Temperature shadow IIR order. BMP5XX_BEFORE_IIR_FILTER or BMP5XX_AFTER_IIR_FILTER"""

        pressure_fifo_iir = rwbit_class(BMP5XX_REG_DSP_CONFIG, 6)
        """Pressure FIFO IIR order. Must be BMP5XX_BEFORE_IIR_FILTER or BMP5XX_AFTER_IIR_FILTER"""

        temperature_fifo_iir = rwbit_class(BMP5XX_REG_DSP_CONFIG, 4)
        """Temperature FIFO IIR order. BMP5XX_BEFORE_IIR_FILTER or BMP5XX_AFTER_IIR_FILTER"""

        iir_flush_forced = rwbit_class(BMP5XX_REG_DSP_CONFIG, 2)
        """Use FORCED mode for IIR filter flush."""

        output_data_rate = rwbits_class(5, BMP5XX_REG_ODR_CONFIG, 2)
        """Output data rate. Must be one of the ODR constants."""

        command = rwbits_class(8, BMP5_REG_CMD, 0)  # command register
        """Command register"""

        def __init__(self, spi_device=None, i2c_device=None):
        ...
        def reset(self) -> None:
            ...
            # suggestions by Bosch to check NVM and INT_STATUS.por
            # wasn't working in SPI driver but other functionality
            # seems fine.
